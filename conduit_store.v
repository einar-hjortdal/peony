...
fn conduit_store_update(mut app App, mut ctx Context, id_bin []u8, ph StoreRequestHygienised) veb.Result {
	...
	if ph.name != none || ph.default_locale_id != none || ph.default_region_id != none
		...
		model_store_update(mut tx, id_bin, ph) or {
			tx.rollback() or {}
	...
	if ph.locale_ids != none {
		model_store_locales_update(mut tx, id_bin, ph.locale_ids_bin) or {
			tx.rollback() or {}
	...
	if currency_codes := ph.currency_codes {
		model_store_currencies_update(mut tx, id_bin, currency_codes) or {
			tx.rollback() or {}
