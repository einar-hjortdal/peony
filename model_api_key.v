...
fn model_api_key_create(mut tx firebird.Transaction, api_key ID, name string, sales_channel_id ID) ! {
	tx.execute('INSERT INTO api_key (id, name, sales_channel_id) VALUES (?, ?, ?)',
		api_key.bytes(), name, sales_channel_id.bytes())!
}

struct APIKeyRetrieveParams {
	ids               ?[]ID
...
fn model_api_key_retrieve_conditions(p APIKeyRetrieveParams) (string, []firebird.Value) {
...
fn model_api_key_retrieve_count(mut tx firebird.Transaction, p APIKeyRetrieveParams) !i64 {
	conditions, params := model_api_key_retrieve_conditions(p)
...
fn model_api_key_retrieve(mut tx firebird.Transaction, p APIKeyRetrieveParams) ![]APIKey {
	conditions, mut params := model_api_key_retrieve_conditions(p)
...
fn model_api_key_delete(mut tx firebird.Transaction, api_key ID) ! {
